...
export function createSyncManager({ getState, replaceState, requestRender, notify }) {
  const config = window.FORGE_SYNC_CONFIG || {};
  ...
  function renderPanel() {
    if (!configured) {
      return `<section class="panel sync-panel"><div class="panel-header"><div><span class="eyebrow">Laptop and iPhone</span><h2>Cloud sync needs its connection values</h2><p>Open <strong>config.js</strong>, enter the Supabase Project URL and Publishable Key, then republish the repository. Local saving remains active.</p></div><span class="status-pill modify">Setup needed</span></div></section>`;
    }
    if (!user) {
