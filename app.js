...
const EXPANDED_KEY = "forge.training.expanded.v1";
...
let toastTimer;
let syncManager;
const expandedExercises = loadExpandedExercises();
...
function render() {
  ...
  app.innerHTML = `
    <header class="topbar">
      ...
      <div id="sync-status-region" class="sync-status-region">${syncManager ? syncManager.renderStatus() : `<span class="save-indicator"><span class="save-dot"></span>Local auto-save on</span>`}</div>
    </header>
...
function renderExercise(exercise, readinessResult, recommendation) {
  ...
  return `
    <article class="exercise-card ${completed ? "completed" : ""} ${modified ? "modify" : ""} ${expandedExercises.has(exercise.id) ? "open" : ""}" data-exercise-card="${exercise.id}">
      <div class="exercise-summary">
        ...
        <div class="load-prescription"><span class="load-label">Recommended</span><strong class="load-number">${isLoad ? (prescribedLoad ? formatNumber(prescribedLoad) : "Enter") : "Track"}</strong><span>${isLoad ? state.program.settings.units : escapeHtml(exercise.metric)}</span></div>
...
function renderSettings() {
  ...
  return `
    ...
    <div id="sync-panel-region">${syncManager ? syncManager.renderPanel() : ""}</div>
...
function handleClick(event) {
  ...
  if (action === "toggle-exercise") toggleExerciseCard(target);
...
function usesLoad(exercise) {
  return LOAD_METRIC.test(exercise.metric)
    || /medicine ball/i.test(exercise.exercise)
    || exercise.percent1RM > 0
    || (state.program.settings.startingLoads[exercise.exercise] || 0) > 0;
}
...
function renderSyncUI() {
  const statusRegion = document.querySelector("#sync-status-region");
  if (statusRegion && syncManager) statusRegion.innerHTML = syncManager.renderStatus();
  const panelRegion = document.querySelector("#sync-panel-region");
  if (panelRegion && syncManager) panelRegion.innerHTML = syncManager.renderPanel();
}

function toggleExerciseCard(button) {
  const card = button.closest(".exercise-card");
  const exerciseId = button.dataset.exercise;
  if (!card || !exerciseId) return;
  const willOpen = !card.classList.contains("open");
  card.classList.toggle("open", willOpen);
  button.setAttribute("aria-expanded", String(willOpen));
  if (willOpen) expandedExercises.add(exerciseId);
  else expandedExercises.delete(exerciseId);
  try {
    sessionStorage.setItem(EXPANDED_KEY, JSON.stringify([...expandedExercises]));
  } catch (error) {
    console.warn("Expanded exercise state could not be saved.", error);
  }
}

function loadExpandedExercises() {
  try {
